...
const sendWithResend = async (recipient, subject, html) => {
    ...
    try {
        const data = await resend.emails.send({
            from: 'Football Tips Kenya <info@footballtips.ke>',
            to: [recipient],
            replyTo: 'support@footballtips.ke',
            subject,
            html
        });
        return data;
...
const sendMailErooMails = async (recipient, subject, html) => {
    try {
        ...
        data.append('from', 'Football Tips Kenya <info@footballtips.ke>');
        data.append('to', `<${recipient}>`);
        data.append('reply_to', '<support@footballtips.ke>')
